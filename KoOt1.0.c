	...
	int main()
	{
		...
		char SecretKey1[] = "!$*(gi%~.,&fuam4^<[}\3em8os#*-" , SecretKey2[] = "gR3$7(~G `.<_&@!\%1*9Azf2-+:;\"P" ;   

		                
		/* "Decryption" is pointing to Encrypted.txt file in which encrypted data is saved after encryption   */              
		FILE *Decryption ;                           
...
//This case : Encryption		
		{   case 1: printf ( "\n Enter the Text to be Encrypted\n" ) ;         // Entering PlainText
                    ...
                    for (i=0 ; i < KeyLen ; i++ )             // Or function on user key with our secret key
	                {  
...
// This case : Decryption		
            case 2: 
			        Decryption = fopen ( "Encrypted.txt" , "rb" ) ;
     			    ...
     			    for (i = 0 ; !feof(Decryption) ; i++ )      // Inputing encrypted data
...
// To XOR characters in text array with characters in Key array  ; for encryption mode=0 ; for decoding mode=1
	...
	{	
	    ...
	    {  for (j = 0 , i = 0 ; j < KeyLen ; j++ )          // XORing whole plaintext with 1st char of key then with 2nd char , so on 
	       {   while ( i < TextLen )
		       {   text[i] = text[i] ^ Key[j] ;
	    	       i++;
	           }
	    ...
	    {   
	    	for (j = 0 , i = 0 ; j < KeyLen ; j++ )   // Decoding
	        {   while ( i < TextLen )
		        {     text[i] = text[i] ^ Key[j] ;
	    	          i++ ;
	            }
